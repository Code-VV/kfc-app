...
<div class="container_common1 contract bold">
    <div class="tabsList flex" v-if="!$no_fb">
        ...
        <!-- <span class="span bor_r bor_t bor_r bor_b c444" @click="goFabi">{{i18n.fb}}</span> -->
    ...
    <div class="contractTops relative mar_t_15" :class="{'mar_t_15':!$no_fb}">
        <div class="titlesBox flex c333" @click="openPoup">
            <img src="../../assets/images/home/mores_1.png" alt srcset class="mores" />
            <span class="span">{{currentName}}</span>
            <span :class="currentCoinInfo.updown>0?'green_text green_bg_a':currentCoinInfo.updown<0?'orange_text red_bg_a':'dark_text dark_bg_a'" class="gain_span">{{currentCoinInfo.updown*100|toFixedRate(2)}}%</span>
        </div>
        <div class="iconsBox">
            <img src="../../assets/images/home/transactionIcon.png" alt srcset class="icons" @click="goQuotesDetail" />
            ...
            <mores @close="close1" v-if="isShowMore" class="absolute right0 top22"></mores>
        </div>
    </div>
    <!-- 合约操作 -->
    <div class="transactionDetail">
        <div class="leftBox">
            <div class="btnBox">
                <span class="btn btn1" :class="{'cur':operationType=='open'}" @click="operation('open')">
                    {{i18n.mr}}
                    <img src="../../assets/images/home/green_bg.png" alt srcset v-if="operationType=='open'" />
                    <img :src="isNight=='night-theme'? require('../../assets/images/home/dark_bg1.png'):require('../../assets/images/home/dark_bg.png')" alt srcset v-else class="darkBg" />
                </span>
                <span class="btn" @click="operation('close')" :class="{'cur':operationType=='close'}">
                    {{i18n.mc}}
                    <img src="../../assets/images/home/orange_bg.png" v-if="operationType=='close'" />
                    <img :src="isNight=='night-theme'? require('../../assets/images/home/dark_bg1.png'):require('../../assets/images/home/dark_bg.png')" alt srcset v-else />
                </span>
            </div>
            <div class="entrustsBox">
                <div class="entrusts flex">
                    <div class="items" @click="priceShow=true">
                        <span class="size12 span c222" v-if="priceType==1"> {{i18n.sj}}</span>
                        <span class="size12 span c222" v-if="priceType==2"> {{i18n.xj}}</span>
                        <img src="../../assets/images/home/dropDown.png" alt srcset class="icons" />
                    </div>
                </div>
                <div class="entrusts1 c222 grayBg mar_t_15 c999" v-if="priceType==1">{{i18n.ydqzyj}}</div>
                <div class="entrusts entrusts1 c222 stepBox" v-else>
                    <van-stepper v-model="markPrice" step="0.0001" @blur="blurFun(1)" @focus="focusFun" :min="0" :decimal-length="4" />
                </div>
                <div v-if="priceType==1" class="mar_t_30"></div>
                <div class="size10 mar_t_8 transerPrice" v-else>≈{{(currentCoinInfo.chPrice/currentCoinInfo.nowPrice)*markPrice|SubString(2)}} CNY</div>
                <div class="entrusts entrusts1 flex-between c222" v-if="priceType==1&&operationType=='open'">
                    <!-- <span class="name" v-if="transaction.count">{{transaction.count}}</span>
            <span class="name" v-else>数量</span>-->
                    <input type="number" :placeholder="i18n.je" class="marketCount" v-model="transaction.marketPrice" @blur="blurFun(2)" @focus="focusFun" style="width:75%;background:none" step="0.1" />
                    <span class>USDT</span>
                </div>
                <div class="entrusts entrusts1 flex-between c222" v-else>
                    <!-- <span class="name" v-if="transaction.count">{{transaction.count}}</span>
            <span class="name" v-else>数量</span>-->
                    <input type="number" :placeholder="i18n.sl" class="marketCount" v-model="transaction.marketCount" @blur="blurFun(3)" @focus="focusFun" style="width:75%;background:none" step="0.1" />
                    <span class>{{tokenCur}}</span>
                </div>

                <div class="size10 mar_t_12 transerPrice" v-if="operationType=='close'">{{i18n.ky}} {{assets.balance|SubString(8)}} {{tokenCur}}</div>
                <div class="size10 mar_t_12 transerPrice" v-else>{{i18n.ky}} {{assets.balance|SubString(8)}} USDT</div>
            </div>
            <div class="pad_l_r_10" v-if="operationType=='open'">
                <van-slider v-model="transaction.slideNum" active-color="#38BB96" class="mar_t_25" @change="slideChange">
                    <img src="../../assets/images/home/icon1.png" slot="button" class="handleBtn" />
                </van-slider>
            </div>
            <div class="pad_l_r_10" v-else>
                <van-slider v-model="transaction.slideNum" active-color="#E8054C" class="mar_t_25" @change="slideChange">
                    <img src="../../assets/images/home/icon_2.png" slot="button" class="handleBtn" />
                </van-slider>
            </div>
            ...
            <div class="balanceBox c222">
                <span v-if="assets.balance-markPrice*transaction.marketCount<0&&operationType!='close'">{{i18n.yebz}}</span>
                <div v-else>
                    <div v-if="priceType==1">
                        <span v-if="operationType=='open'">{{i18n.jye}} {{(transaction.marketPrice?transaction.marketPrice:0)|SubString(8)}}USDT</span>
                        <span v-else>{{i18n.jye}} {{currentCoinInfo.nowPrice*transaction.marketCount|toNumber(8)}}USDT</span>
                    </div>
                    <div v-else>
                        <span>{{i18n.jye}} {{markPrice*transaction.marketCount|toNumber(8)}}USDT</span>
                    </div>
                </div>
            </div>
            <div class="operatingBtn">
                <div class="items orange_bg" v-if="operationType=='close'" @click="submitHandle('SELL')">{{i18n.mc}}{{tokenCur}}</div>
                <div class="items green_bg" v-else @click="submitHandle('BUY')">{{i18n.mr}}{{tokenCur}}</div>
            </div>
        </div>
        <div class="rightBox">
            ...
            <div class="contentBox flex1 mar_t_10">
                <div class="flex1 tops">
                    <van-row gutter="20" v-for="(item,i) in opendown" :key="i" @click="setMarkPrice(item.price)" class="relative-index-2 size11 flex1 items c777 pad_r_5">
                        <van-col span="12" class="orange_text">{{item.price |SubStringZreo(currentName.split('/')[0]=='HJGX'?8:4)}}</van-col>
                        <van-col span="12 right_text">{{item.num |SubString(4)}}</van-col>
                    </van-row>
                    <div class="areaBox">
                        <div v-for="(item,i) in opendown" :key="i" :style="{'width':(item.num/maxSell)*70+'%'}" class="orange items"></div>
                    </div>
                </div>
                <div class="pad_t_b_20">
                    <div class="price size11 c333">
                        <span class="green_text size16 mar_r_11">{{currentCoinInfo.nowPrice|SubString(4)}}</span>
              ...
              <span class="mar_l_4">{{zhishuPrice|SubStringZreo(4)}}</span>
                ...
                <div class="flex1 tops">
                    <van-row gutter="20" v-for="(item,i) in openup" :key="i" @click="setMarkPrice(item.price)" class="relative-index-2 size11 items pad_r_5 c777">
                        <van-col span="12" class="green_text">{{item.price |SubStringZreo(currentName.split('/')[0]=='HJGX'?8:4)}}</van-col>
                        <van-col span="12 right_text">{{item.num |SubString(4)}}</van-col>
                    </van-row>
                    <div class="areaBox">
                        <div v-for="(item,i) in openup" :key="i" :style="{'width':(item.num/maxBuy)*70+'%'}" class="green items"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="contranctList">
        ...
        <div class="listBox" v-if="entrustList.length>0">
            <div v-for="(item,i) in entrustList" :key="i">
                <transactionItem :list="item" type="entrust" :n="i" @cancelorder="cancelorder"></transactionItem>
            </div>
        </div>
        <div v-else>
    ...
    <van-dialog v-model="priceShow" :cancel-button-text="$t('add.qx')" class="transactionPop" :title="i18n.xzlx" show-cancel-button :showConfirmButton="false">
        <div class="pad_15 c111">
            <p @click="selectPrice(2)" class="center_text linehei40 size14" :class="{'blue_text':priceType=='2'}">{{i18n.xj}}</p>
            <p @click="selectPrice(1)" class="center_text linehei40 size14" :class="{'blue_text':priceType=='1'}">{{i18n.sj}}</p>
        </div>
    </van-dialog>
    ...
    methods: {
        ...
        setStyle() {
            if (this.$store.state.nowThemeInfo == "night-theme") {
                ...
                $(".contract")
                    .find(".van-stepper__minus")
                    .css("color", "#fff");

                $(".contract")
                    .find(".van-stepper__plus")
                    .css("color", "#fff");

                $(".contract")
                    .find(".van-stepper__input")
                    .css("color", "#fff");

                $(".contract")
                    .find(".grayBg")
                    .css("background-color", "#272e48");
                // 加和减的按钮颜色
            } else {
                $(".contract")
                    .find(".van-dialog__header")
        ...
        submitHandle(type) {
            ...
            let data = {
                ...
                percentageCount: this.$util.toNumber(this.transaction.slideNum / 100, 8),
                matchFee: this.priceType == 1 && this.operationType == "open" ?
                    this.transaction.marketPrice : this.$util.toNumber(marketCount * this.markPrice)
            };

            // this.$util.showLoading();
            this.disableClick = true;
            this.$post("entrust/entrust/setEntrust", data).then(res => {
                // 延迟启用点击
                setTimeout(() => {
