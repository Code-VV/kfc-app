...
export default {
  created() {
    ...
    for (let i = 0; i < cityList.length; i++) {
      // cityList[i].name.indexOf('(')
      // cityList[i].name.indexOf(')')
      cityList[i].name = cityList[i].name.substring(
        cityList[i].name.indexOf("(") + 1,
        cityList[i].name.indexOf(")")
      );
      // cityList[i].name=0
  ...
  methods: {
    ...
    init() {
      this.setnavTitle(this.$t("xzgj.xzgj"));
      this.setbackRouter("");
...
/deep/.van-search__content,.van-cell__value{
  background-color: #45475B;
}
.van-cell__value{
  background-color: #45475B;
}
.night_style .van-cell[data-v-5e0c2ef1] {
