<template>
  <div class="container_common weixinCard">
    ...
    <numberKeyboard type="weixin" ref="mychild" @submitEvent="submitEvent" />
...
export default {
  ...
  watch: {
    "userInfo.wechatName"(val) {
      this.userInfo.wechatName = val.replace(/\s+/g, "");
    }
  },
  methods: {
    ...mapActions([
    ...
    submit() {
      if (!this.userInfo.wechatName) {
        this.Toast("请输入账户名");
        return;
      }
      if (!this.userInfo.payWechat) {
        this.Toast("请上传收款二维码");
        return;
      }
	  this.submitEvent("") ;
    ...
    submitEvent(val) {
      var data = {
        code: val,
        name: this.userInfo.wechatName,
        file: this.userInfo.payWechat
      };
      var fd = new FormData();
    ...
    getImgSrc(e) {
      this.userInfo.payWechat = e;
    },
    //删除图片
    close(index) {
      this.userInfo.payWechat = "";
    }
  },
  destroyed() {}
};
