<template>
  <div class="bankCard container_common">
    ...
    <van-cell-group class="formBox mar_t_10">
      <van-field v-model="userInfo.uname" :label="$t('skfs.xm')" placeholder="姓名" readonly />
      <van-field v-model="userInfo.bankCard" :label="$t('skfs.yhk')" type="number" />
      <SinglePicker

        @pickerIdx="county_pickerIdx"
        :list="bankList"
        style="width:100%"
        :index="index"
      ></SinglePicker>
      <van-field v-model="userInfo.bankAddress" :label="$t('skfs.zh')" />
    </van-cell-group>
    <van-button
    ...
    <numberKeyboard type="bank" ref="mychild" @submitEvent="submitEvent" />
...
export default {
  ...
  watch: {
    "userInfo.uname"(val) {
      this.userInfo.uname = val.replace(/\s+/g, "");
    },
    "userInfo.bankCard"(val) {
      this.userInfo.bankCard = val.replace(/\s+/g, "");
    },
    "userInfo.bankAddress"(val) {
      this.userInfo.bankAddress = val.replace(/\s+/g, "");
    }
  },
  methods: {
    ...mapActions([
    ...
    getUserInfo() {
      this.$get("member/member/getMember", {
        ...
        if (res.status == "FAILED") {
          ...
          var index = this.bankList.findIndex(
            val => val.text == res.result.bankName
          );
          this.index = index;
        }
      });
    ...
    init() {
      this.setnavTitle(this.$t('skfs.yhkbd'));
      this.setbackRouter("");
    ...
    county_pickerIdx(idx) {
      this.index = idx;
      this.userInfo.bankName = this.bankList[idx].text;
    },
    submit() {
    ...
    submitEvent(val) {
      ...
      var data = {
        member: this.$store.state.userId,
        bankUserName: this.userInfo.uname,
        bankCard: this.userInfo.bankCard,
        bankName: this.userInfo.bankName,
        bankAddress: this.userInfo.bankAddress,
        code: val
      };
      this.$util.showLoading();
      this.$post1("member/member/setBankInfo", data).then(res => {
