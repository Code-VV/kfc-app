    ...
    <div class="transferTops flex mar_t_10">
        ...
        <van-cell-group class="center_formBox">
            <van-cell class="c333" @click="openPoup(1)">
                <span slot="default" v-if="formType==-1">{{$t('transfer.zczm')}}</span>
          ...
          <span slot="default" v-if="formType==4">OTC账户（USDT）</span>-->
            ...
            <van-cell class="c333" @click="openPoup(2)">
                <span slot="default" v-if="toType==-1">{{$t('transfer.zrzh')}}</span>
        ...
        <div class="rightIcons" @click="transferTitHand">
            ...
            <img src="../../assets/images/home/transfer_1.png" alt srcset class="icons" />
        </div>
    </div>
    <div class="mar_t_24 darkLine"></div>
    <van-cell-group class="withdrawCoinForm c222">
        <van-cell :title="$t('transfer.bz')" is-link :value="currency" to="/assets/chooseCoin?type=transfer" />
        <van-field v-model="coinNum" center clearable type="number" :label="$t('transfer.hzsl')" :placeholder="this.$t('transfer.zdkhz')+transferInfo.amount+currency">
            <span slot="right-icon" class="allCoinNum" @click="transferAll">{{$t('transfer.qbzc')}}</span>
        </van-field>
    </van-cell-group>
    <van-button type="info" @click="transferHandle" class="submit">{{$t('transfer.hz')}}</van-button>
    <div class="tipDetail">
        <p>{{$t('transfer.zyj')}}</p>
    </div>
    <van-dialog v-model="priceShow" class="transactionPop" title :showConfirmButton="false" :closeOnClickOverlay="true">
        <div class="pad_15" v-if="transferType==1">
            <p v-for="(item,i) in chooseList" :key="i" @click="selectPrice(item,i)" class="center_text linehei40 size14 " :class="formType==i?'blue_text':'c333'">{{item}}</p>
        </div>
        <div class="pad_15" v-else>
            <p v-for="(item,i) in chooseList" :key="i" @click="selectPrice(item,i)" class="center_text linehei40 size14" :class="toType==i?'blue_text':'c333'">{{item}}</p>
        </div>
    </van-dialog>
</div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            chooseList: [this.$t('transfer.ctzh'),this.$t('transfer.bbzh'),this.$t('transfer.hyzh'),this.$t('transfer.fbzh'),]
        };
    },
    computed: {
        userId() {
...
.withdrawCoin {
    ...
    .transferTops {
        .van-cell {
            background: url(../../assets/images/home/dropDown.png) no-repeat right center !important;
    ...
    .pwdModel {
        ...
        .van-icon-close:before {
            content: "";
            background: url("../../assets/images/home/close.png") no-repeat left center;
