...
<div class="assets container_common1 flex flex_column">
    ...
    <van-row gutter="20" class="accountBox bor_b">
        <van-col offset="2" span="4">
            <div class="items c444" :class="{'cur':curType==1}" @click="switchTab(1)">
                {{i18n.bb}}
                <i class="borders"></i>
            </div>
        </van-col>
        <van-col span="4">
            <div class="items c444" :class="{'cur':curType==2}" @click="switchTab(2)">
                {{i18n.hy}}
                <i class="borders"></i>
            </div>
        </van-col>
        ...
        <van-col span="4">
            <div class="items c444" :class="{'cur':curType==4}" @click="switchTab(4)">
                {{i18n.ct}}
                <i class="borders"></i>
            </div>
        </van-col>
    ...
    <div class="accountDetail flex1" v-if="assetList.balances&&assetList.balances.length>0">
        ...
        <div class="lists">
            <assetItem v-for="(item,i) in assetList.balances" :key="i" :list="item" :type="curType"></assetItem>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        initBalanceList() {
            ...
            //     ? "BALANCE_HY"
            //     : "BALANCE_FB";
            let coinType = this.curType == 1 ? "SPOT" : this.curType == 2 ? "CONTRACT" : this.curType == 3 ? "FB" : this.curType == 4 ? "ASSETS" : "RELEASE";
            this.$get("member/balance/getBalanceList", {
                ...
                if (res && res.status == "SUCCEED") {
                    if (this.curType == 5) {
                        res.result.balances = res.result.balances.filter(elem => elem.currency == "FVC");
                    }
