  ...
  <div class="withdrawCoin container_common">
    ...
    <van-cell-group class="withdrawCoinForm c222">
      <van-field
        ...
        clearable
        :label="i18n.tbdz"
        :placeholder="i18n.qsrtbdz"
      >
        <!-- <img src="../../assets/images/home/scan.png" alt srcset slot="right-icon" class="scan" @click="openScan" /> -->
      </van-field>
      <van-field
        v-model.trim="coinNum"
        center
        clearable
        type="number"
        :label="i18n.sl"
        :placeholder="i18n.zdtbsl + wallets.money + currency"
      >
        <span slot="right-icon" class="allCoinNum" @click="allCoinNum">{{
          i18n.qbtq
        }}</span>
      </van-field>
    </van-cell-group>
    <van-cell-group class="withdrawCoinForm moneyBox">
      <van-cell :title="i18n.sxf">
        <span slot="default"
          >{{ wallets.fee | SubString(4) }} {{ currency }}</span
        >
      </van-cell>
      <van-cell :title="i18n.sjdzsl">
        <span slot="default"
          >{{ wallets.actuals | SubString(4) }} {{ currency }}</span
        >
      </van-cell>
    </van-cell-group>
    <van-button
      ...
      disabled
      class="submit"
      >{{ i18n.jztb }}</van-button
    >

    <van-button
      v-else
      style="width: 100%; background-color: #169e78"
      type="info"
      @click="showPopup"
      class="submit"
      >{{ i18n.tb }}</van-button
