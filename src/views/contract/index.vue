...
<div id="container_common111" class="container_common1 contract bold">
    ...
    <div class="contractTops relative ">
        ...
        <div class="iconsBox">
            <router-link :to="{path:'/quotes/quotesDetail',query:{type:'contract'}}">
                <img src="../../assets/images/home/transactionIcon.png" alt srcset class="icons" />
            </router-link>
    ...
    <van-row gutter="20" class="tabBox size12 center_text c777 contract_style1">
        <van-col span="6">
            <div>{{i18n.fxd}}</div>
            <div class="mar_t_10" v-if="(risk.risk-0)==0">{{(risk.risk*100)|SubString(2)}}%</div>
            <div class="mar_t_10 c_green bold" v-else-if="risk.risk<=0.5">{{(risk.risk*100)|SubString(2)}}%</div>
            <div class="mar_t_10 c_red bold" v-else-if="risk.risk>0.5">{{(risk.risk*100)|SubString(2)}}%</div>
        </van-col>
        <van-col span="12">
            <div>{{i18n.syl}}</div>
        ...
        <div class="leftBox">
            ...
            <div class="entrustsBox">
                <div class="entrusts flex">
                    <div class="items" @click="priceShow=true">
                        <span v-show="currentPriceType=='1'" class="size12 c222">{{i18n.sj}}</span>
                        <span v-show="currentPriceType=='2'" class="size12 c222">{{i18n.xj}}</span>
            ...
            />-->
                    <div v-show="priceShow" class="ganggan c333">
                        ...
                        <div class="con">
                            <van-picker :confirm-button-text="$t('add.qd')" :cancel-button-text="$t('add.qx')" show-toolbar :title="i18n.xzlx" :columns="actions" :item-heigh="50" @cancel="priceShow=false" @confirm="selectPrice" />
                        </div>
                    </div>
                    ...
                    <div v-show="gangganShow" class="ganggan c333">
                        <!-- <div class="size16 bold hei40 linehei40 a_center">选择杠杆</div> -->
                        <div class="con">
                            <van-picker :confirm-button-text="$t('add.qd')" :cancel-button-text="$t('add.qx')" show-toolbar :title="i18n.xzgg" :columns="columns" :item-heigh="50" @cancel="selGanggancLE" @confirm="selGangganOk" />
               ...
               <van-row  class="mar_b_10" >
                ...
                <!-- 市价 -->
                <div v-show="currentPriceType=='1'" class="entrusts entrusts1 pad_l_10">
                    <span class="name">{{i18n.sj}}</span>
                    <span class="rightBorder" @click="currentPriceType='2'">{{i18n.sj}}</span>
                </div>
                <!-- 限价 -->
                <div v-show="currentPriceType=='2'" class="entrusts entrusts1" style="padding-left:0">
                    <div class="name a_re" style="width:100%;height:100%">
                        <input class="inp c666" type="number" :placeholder="i18n.jg" v-model="price" @blur="blurFun" @focus="focusFun" />
                        <div class="a_ab c666 right5 top5 size12">{{coinMain}}</div>
                    </div>
                    <span class="rightBorder2 c111" @click="currentPriceType='1'">{{i18n.sj}}</span>
                </div>
            ...
            <span class="flex1"></span>-->
                    ...
                    <!-- 开仓手数输入 -->
                    <input v-show="operationType=='open'" class="inppp c666" type="number" :disabled="disabled" :class="{'bg_ededed':disabled}" v-model="shouNum" @blur="blurFun" @focus="focusFun" @input="changeShoushu(shouNum)" :placeholder="i18n.sl" />
                    <!-- 平仓手数输入 -->
                    <input v-show="operationType=='close'" class="inppp c666" type="number" :disabled="disabledPingcang" :class="{'bg_ededed c999':disabledPingcang}" v-model="shouNumPing" @blur="blurFun" @focus="focusFun" @input="changeShoushuPing(shouNumPing)" :placeholder="i18n.sl" />

                    <span class="a_ab right10 top5 c111">{{i18n.s}}</span>
                </div>
            </div>
            <div v-if="operationType=='open'" class="size11 c777 mar_t_5 bold">= {{contractMul * handerNan(shouNum) | SubString(4)}}{{currentName.split('/')[0]}}</div>
            <div v-else class="hei20"></div>

            <!-- 开仓进度条 -->
            <div class="pad_l_r_10" v-show="operationType=='open'">
                <van-slider :disabled="disabled" v-model="sliderNum" @input="changeSlide(sliderNum)" active-color="#38BB96" class="mar_t_20">
            ...
            <div class="pad_l_r_10" v-show="operationType=='close'">
                <van-slider :disabled="disabledPingcang" v-model="sliderNumPing" @input="changeSlidePing(sliderNumPing)" active-color="#38BB96" class="mar_t_20">
                    <img src="../../assets/images/home/icon1.png" slot="button" class="handleBtn" />
                </van-slider>
            </div>
            ...
            <div class="balanceBox c222">
                ...
                <div class="items">
                    <span>{{i18n.dqzc}}</span>
                    <span>{{(assets.tokenBalance-0)+(assets.tokenBlockedBalance-0) | SubStringZreo(4)}}</span>
                </div>
            </div>
        ...
        <div class="rightBox">
            ...
            <div class="contentBox flex1 mar_t_10">
                <div class="flex1 tops">
                    <van-row gutter="20" v-for="(item,i) in opendown" :key="i" class="size11 flex1 items pad_r_5 c777">
                        <van-col span="12" class="orange_text">{{item.price |SubStringZreo(4)}}</van-col>
              ...
              >{{currentPrice1|SubStringZreo(4)}}</div>-->
                        <div :class="currentPrice>=prevePrice?'green_text':'orange_text'" class="size16 mar_r_6">{{currentPrice|SubStringZreo(4)}}</div>
                        <!-- <div
                :class="currentPrice>=prevePrice?'green_text':'orange_text'"
                class="size16 mar_r_6"
              >{{currentPrice2|SubStringZreo(4)}}</div>-->
                    ...
                    <div class="size11 mar_t_6 price c333">
                        <span>{{i18n.zsjg}}</span>
                        <span class="span mar_l_4 c222">{{zhishuPrice|SubStringZreo(4)}}</span>
                    </div>
                </div>

                <div class="flex1 tops">
                    <van-row gutter="20" v-for="(item,i) in openup" :key="i" class="size11 items pad_r_5 c777">
                        <van-col span="12" class="green_text">{{item.price |SubStringZreo(4)}}</van-col>
                        <van-col span="12 right_text">{{item.num |SubString(4)}}</van-col>
                    </van-row>
                    <div class="areaBox">
        ...
        <div v-show="tabIndex==0">
            ...
            <div class="listBox" v-if="chicangList.length>0">
                <div v-for="(item,i) in chicangList" :key="i">
                    <contractItem @res="result" :list="item" :type="tabIndex==1?'entrust':''" :ensure="ensure"></contractItem>
                </div>
            </div>
    ...
    <van-popup v-model="show1" style="background:none">
        <div class="wrap">
            <div v-show="isKaiduo" class="size16 c333 a_center hei60 linehei60">{{i18n.kd}}</div>
            <div v-show="!isKaiduo" class="size16 c333 a_center hei60 linehei60">{{i18n.kk}}</div>
...
contractMulId: "537b5f5cb0ca0ae7ff2a5af8af59cdfd"
          leverId: "6e56dfdb7a7c40da4879f05be77b80a6"-->
                ...
                <van-row class="hei22 linehei22">
                    ...
                    <van-col :span="12" class="c333 a_right">{{currentDta.pairsName}}{{i18n.yx}}</van-col>
                </van-row>

                <van-row class="hei22 linehei22">
                    <van-col :span="12" class="c999">{{i18n.jg}}</van-col>
                    <van-col :span="12" class="c333 a_right">{{currentDta.price| SubStringZreo(4)}}</van-col>
                </van-row>

                <van-row class="hei22 linehei22">
                    <van-col :span="12" class="c999">{{i18n.sl}}</van-col>
                    <van-col :span="12" class="c333 a_right">{{currentDta.contractHands}}</van-col>
                </van-row>

                <van-row class="hei22 linehei22">
                    <van-col :span="12" class="c999">{{i18n.gg}}</van-col>
                    <van-col :span="12" class="c333 a_right">{{currentGangganType}}{{i18n.b}}</van-col>
                </van-row>
                ...
                <van-row class="hei22 linehei22">
                    ...
                    <!-- 保证金=手数×合约乘数×下单价格/杠杆 -->
                    <van-col :span="12" class="c333 a_right">{{(currentDta.contractHands * contractMul * currentDta.price/currentGangganType)|SubStringZreo(4)}}</van-col>
                </van-row>
            </div>
        ...
        setBgColor() {
            ...
            if (this.isNight == "night-theme") {
                ...
                setTimeout(() => {
                    ...
                    dom1.style.backgroundColor = "#272A3F";
                    dom3.style.backgroundColor = "#272A3F";
                ...
                if (res && res.status == "SUCCEED") {
                    ...
                    if (data.length > 0) {
                        ...
                        if (data.length == 2) {
                            ...
                            this.kongdanData = data.find(ele => ele.tradeType == "OPEN_DOWN");
                            ...
                            let kongdanData = data.find(ele => ele.tradeType == "OPEN_DOWN");
