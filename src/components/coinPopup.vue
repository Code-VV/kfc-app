<template>
  <div class="coinPopup">
    ...
    >
      <div class="titles c000" :class="{ pad_b_21: type == 'contract' }">
        {{ type == "contract" ? $t("quotes.hy") : $t("quotes.bb") }}
      </div>
      <div class="subContent" v-if="true">
        <div class="subTabList">
          <span
            ...
            :key="i"
            :class="{ cur: curSubType == item.type }"
            @click="switchSubTab(item.type)"
          >
            {{ item.name }}
            <i class="borders"></i>
          </span>
      ...
      <div class="marketList">
        <div
          class="listItems bor_b"
          v-for="(item, i) in pairsList"
          :key="i"
          @click="switchCoinName(item.pairsName)"
          :class="{ border_none: i == pairsList.length - 1 }"
        >
          <div class="titleBox c222">
            {{ item.tokenCur }}
            <span class="marks c999">/ {{ item.mainCur }}</span>
            <span v-show="type == 'contract'" class="size12 c999"> {{i18n.yx}}</span>
          </div>
          <div class="right_box">
            <!-- <span :class="item.updown>0?'green_text':'orange_text'">{{item.price|SubString(2)}}</span> -->
            <span :class="item.updown > 0 ? 'green_text' : 'orange_text'">{{
              item.price | SubString3(2, 2)
            }}</span>
          </div>
        </div>
      </div>
