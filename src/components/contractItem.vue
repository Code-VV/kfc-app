  ...
  <div class="contractItem">
    ...
    <div class="titleBox flex-between bor_b c666">
      ...
      <span v-show="currentHistoryType == '1'">
        ...
        <span v-show="list.tradeType == 'OPEN_UP'" class="green_text">
          <span class="">
            {{i18n.kd}} {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X
          </span>
        </span>

        <span v-show="list.tradeType == 'CLOSE_UP'" class="green_text">
          <span class="red_text">
            {{ list.closeType == "BURST" ? "多强平" : "平多" }}
            {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X
          </span>
        </span>

        <span v-show="list.tradeType == 'OPEN_DOWN'" class="green_text">
          <span class="red_text">
            {{i18n.kk}} {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X
          </span>
        </span>

        <span v-show="list.tradeType == 'CLOSE_DOWN'" class="green_text">
          <span>
            {{ list.closeType == "BURST" ? "空强平" : "平空" }}
            {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X
          </span>
        </span>
      ...
      <span v-show="currentHistoryType != '1'">
        <span v-show="list.tradeType == 'OPEN_UP'" class="green_text"
          >{{i18n.dd}} {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X</span
        >
        <span v-show="list.tradeType == 'OPEN_DOWN'" class="green_text red_text"
          >空开 {{ list.pairsName }}{{i18n.yx}} {{ list.leverNum | toFixed(2) }}X</span
        >
      </span>
      <!-- 持仓 -->
      <!-- <span v-else>
      ...
      <span v-if="type == 'entrust'">
        <span v-if="subType == 'revoke'">{{i18n.ycx}}</span>
        <span v-else>{{i18n.wcj}}</span>
      </span>
      <img
    ...
    <div class="listContent bor_b">
      <div v-if="type == 'entrust' || type == 'history'">
        <van-row gutter="20" class="lists c666">
          <van-col span="8" class="markers">
            <div v-show="type == 'entrust'">
              {{ list.contractHands | SubString(4) }}
            </div>
            <div
              v-show="type == 'history'"
              :class="currentHistoryType == 2 ? '' : 'bold_num size14'"
            >
              {{ list.contractHands | SubString(4) }}
            </div>

            <!-- 撤销的 -->
            <div v-if="currentHistoryType == 2" class="mar_t_6">{{i18n.wtsl}}</div>
            <div v-else class="mar_t_6">
              {{ type == "entrust" ? "委托数量" : "成交数量" }}
            </div>
          </van-col>
          <van-col span="8" class="center_text">
            ...
            <div v-else>
              ...
              <div :class="type == 'history' ? 'bold_num size14' : ''">
                <span v-if="xiabiao % 2 == 0">{{
                  list.price | SubStringZreo(4)
                }}</span>
            ...
            <div v-if="currentHistoryType == 2" class="mar_t_6">{{i18n.wtjg}}</div>
            <div v-else class="mar_t_6">
              {{ type == "entrust" ? "委托价格" : "成交价格" }}
            </div>
          </van-col>
          <van-col span="8" class="right_text markers">
            <div :class="type == 'history' ? 'bold_num size14' : ''">
              <div v-if="list.matchFee == '--'" class="dark_text">--</div>
              ...
              <div v-else>
                ...
                >
                  <span
                    class="dark_text"
                    v-if="
                      list.tradeType == 'OPEN_DOWN' ||
                      list.tradeType == 'OPEN_UP'
                    "
                  >
                    ---
                  </span>
      ...
      <div v-else>
        <van-row gutter="20" class="lists size15 c666">
          <van-col span="8" class="c222 bold">{{
            list.avePrice | SubStringZreo(4)
          }}</van-col>
          <van-col
            span="8"
            class="center_text"
            :class="list.unProfitLoss > 0 ? 'c_green' : 'c_red'"
            >{{ list.unProfitLoss > 0 ? "+" : ""
            }}{{ list.unProfitLoss | SubStringZreo(4) }}</van-col
          >
          <van-col
            span="8"
            class="right_text"
            :class="list.profitUp > 0 ? 'c_green' : 'c_red'"
            >{{ list.profitUp > 0 ? "+" : ""
            }}{{ (list.profitUp * 100) | SubStringZreo(2) }}%</van-col
          >
        </van-row>

        <van-row gutter="20" class="lists mar_t_6 c666">
          <van-col span="8" class="markers">{{i18n.ccjj}}</van-col>
          <van-col span="8" class="center_text">{{i18n.wsx}}</van-col>
          <van-col span="8" class="right_text markers">{{i18n.syil}}</van-col>
        </van-row>
      </div>
      <div class="lists c666">
        ...
        <div class="tableBox" v-else-if="type == 'history'">
          <div class="items">
            ...
            <span v-if="currentHistoryType == 2" class="markers">{{
              0 | SubString(4)
            }}</span>
            <span v-else class="markers">{{
              list.takeFee | SubString(4)
            }}</span>
          </div>
          <div class="items">
            <span>方式</span>
            <!-- 第二行 -->
            <span v-if="xiabiao % 2 == 0" class="markers">{{
              list.priceType == "CUSTOM_PRICE" ? "限价" : "市价"
            }}</span>
            <!-- 第一行 -->
        ...
        <div class="tableBox" v-else>
          ...
          <div class="items">
            <span>{{i18n.zybzj}}</span>
            <span class="markers">{{ list.margin | SubStringZreo(4) }}</span>
          </div>
          <div class="items">
            <span>{{i18n.wzbzj}}</span>
            <span class="markers"
              >{{ (ensure * 100) | SubStringZreo(2) }}%</span
            >
          </div>
          <div class="items">
            <span>{{i18n.zyjg}}</span>
            <span class="markers">{{
              list.triggerPrice | SubStringZreo(4)
            }}</span>
          </div>
          <div class="items">
            <span>{{i18n.zwjg}}</span>
    ...
    <div v-if="type != 'history' && subType != 'revoke'">
      ...
      <div v-if="type == 'entrust'" class="tabBox1">
        ...
        <div class="btn" @click="chedan(list)">{{i18n.cd}}</div>
      </div>
      <!-- 持仓列表 -->
      <div v-else>
        <div v-if="currentHistoryType == 3" class="tabBox"></div>
        <div v-else class="tabBox c444">
          <div class="items" @click="pingcang(list)">
            <img
              src="../assets/images/home/transaction_1.png"
              alt
              class="imgs"
            />
            <p class="name">{{i18n.pc}}</p>
          </div>
          <div class="items" @click="zhingying(list)">
            <img
              src="../assets/images/home/transaction_2.png"
              alt
              class="imgs"
            />
            <p class="name">{{i18n.zy}}</p>
          </div>
          <div class="items" @click="zhisun(list)">
            <img
              src="../assets/images/home/transaction_3.png"
              alt
              class="imgs"
            />
            <p class="name">{{i18n.zhisun}}</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <van-row class="pad_l_r_15 hei38 linehei38 size14">
          ...
          <van-col :span="12">
            <input
              v-model="price"
              placeholder="价格"
              class="size14 pad_l_8 input3 c666"
              type="number"
            />
          </van-col>
        </van-row>
          ...
          <div class="content_share a_re">
            ...
            <div class="numdata">
              <div class="list_data">
                ...
                <van-row
                  style="color: #2f2bcd; font-weight: 600"
                  class="size14 pad_t_6"
                >
                  <van-col :span="10">{{ pairsName }}永续</van-col>
                  <van-col :span="7">{{ avePrice | toFixed(2) }}</van-col>
                  <van-col :span="7" class="right_text">{{
